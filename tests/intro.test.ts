...
test("keeps first-person intro cards", () => {
  ...
  assert.equal(
    isIntro("I’m a solo founder from the UK. Currently working on CodePrepped"),
    true,
  );
  ...
  assert.equal(
    isIntro("Hey, I'm a builder working on local-first tools"),
    true,
  );
});
...
test("drops retweets and third-person mentions", () => {
  ...
  assert.equal(
    isIntro(
      "I know a solo founder building an AI productivity app who’d be a fun fit",
    ),
    false,
  );
  assert.equal(
    isIntro("Always good meeting another solo founder building and shipping"),
    false,
  );
  assert.equal(isIntro("CFTC says it will issue crypto rules"), false);
  assert.equal(
    isIntro("Looking to connect with more builders & indie hackers!"),
    false,
  );
  assert.equal(
    isIntro(
      "I AM NOT looking to connect with: Corporate climbers, Indie hackers who never fail",
    ),
    false,
  );
});
